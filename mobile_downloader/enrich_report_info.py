...
import time
import datetime
from coord_convert import wgs84_to_gcj02
...
def get_report_location_from_lat_lon(lat, lon):
        ...
        loc_desp = loc_response.json()["GaoDeMiaoShu"]
        return loc_desp


def enrich_report_info(report_info):
        report_info["loc_desp"] = get_report_location_from_lat_lon(report_info["lat"], report_info["lon"])
        report_info["time_str"] = datetime.datetime.fromtimestamp(report_info["time"]).strftime("%Y-%m-%d %H:%M:%S")
        return report_info
